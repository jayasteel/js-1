function pf(j){j.feed.entry.slice(0,2).forEach(({title,link,media$thumbnail})=>{document.querySelector(".n6").innerHTML+=`<li><a href="${link.find(l=>l.rel==="alternate").href}" target="_blank" title="${title.$t}">${media$thumbnail?`<img src="${media$thumbnail.url.replace(/s\d{2,4}-/,"s200-")}" alt="Thumbnail">`:""}<strong>${title.$t}</strong></a></li>`})}
document.querySelector('.u5').innerHTML = `
<style>
