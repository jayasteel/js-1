...
`);
document.getElementById("rD").value=new Date().toLocaleDateString();let ratingValue=0;const stareviews=document.querySelectorAll(".stareviews");stareviews.forEach(star=>{star.addEventListener("click",function(){ratingValue=this.getAttribute("data-value");updateStareviews(ratingValue)}),star.addEventListener("mouseover",function(){const value=this.getAttribute("data-value");updateStareviews(value)}),star.addEventListener("mouseout",function(){updateStareviews(ratingValue)})});function updateStareviews(value){stareviews.forEach(star=>{star.getAttribute("data-value")<=value?star.classList.add("selected"):star.classList.remove("selected")})}function sendReview(){const name=document.getElementById("name").value,rD=document.getElementById("rD").value,reviewText=document.getElementById("reviewText").value,currentUrl=window.location.href;if(0===ratingValue)return alert("Silakan pilih rating bintang!"),!1;const message=`Ulasan Baru:\n\nNama: ${name}\nTanggal: ${rD}\nRating: ${ratingValue} Bintang\nUlasan: ${reviewText}\nURL Halaman: ${currentUrl}`,phone="6281327707780",whatsappUrl=`https://wa.me/${phone}?text=${encodeURIComponent(message)}`;return window.open(whatsappUrl,"_blank"),!1}
